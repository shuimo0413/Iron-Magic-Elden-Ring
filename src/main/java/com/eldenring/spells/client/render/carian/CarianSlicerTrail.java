...
/**
 * 卡利亚迅剑挥砍光轨：跟手记录刃根 / 刃尖世界坐标，画出剑扫过的月牙面，并沿路径刷粒子。
 * <p>
 * 采样发生在 {@link CarianSlicerHandLayer}（模型已经摆好斩击姿势）。
 * 画 ribbon 放在 {@code AFTER_ENTITIES}，用相机空间而不是玩家 PoseStack，避免身体旋转把轨迹拧歪。
 */
@EventBusSubscriber(modid = EldenRingSpellsMod.MOD_ID, value = Dist.CLIENT)
public final class CarianSlicerTrail {

    /**
     * 刃尖路径光带：短、头略宽、尾细，加法光芯。长度单位方块。
     */
     ...
     */
    private static final double SLASH_RESET_DISTANCE_BLOCKS = 2.6;
    ...
    private static final int MAX_SWEEP_SAMPLE_COUNT = 14;

    private static final Map<UUID, PlayerSlashTrail> TRAILS_BY_PLAYER = new HashMap<>();

    private CarianSlicerTrail() {
    }
    ...
    private static final class PlayerSlashTrail {
        ...
        private boolean shouldResetForNewSlash(AbstractClientPlayer player, Vec3 bladeTipWorld) {
            if (player instanceof LocalPlayer) {
                int slashSequenceIndex = CarianSlicerClientHold.slashSequenceIndex();
        ...
        private void render(
            ...
            // 历史点不含「当前头」重复：末点即最新采样；renderOrigin=ZERO 因 PoseStack 已 -camera。
