 ...
 * <p>
 * 可穿透有限个实体（次数见 {@link #PROJECTILE_MAX_ENTITY_HITS}），法术书用 {@code Pierce ×N} 提示。
 * 螺旋半径、角速度、穿透次数改 {@link SpiralShardSpell}。
 */
public class SpiralShardSpell extends EldenRingAbstractSpell {
    ...
    // -------------------------------------------------------------------------
         ...
         * 调大 → 更容易拐弯；调小 → 更接近直飞螺旋。
         */
        public static float PROJECTILE_MAX_TURN_ANGLE_DEGREES_PER_TICK = 2.0f;

        /** 出手后直飞、不追踪的 tick 数。 */
        public static int PROJECTILE_TRACKING_START_DELAY_TICKS = 4;

        /** 索敌锥半角（度）。 */
        public static float PROJECTILE_TRACKING_ACQUIRE_CONE_HALF_ANGLE_DEGREES = 35.0f;
         ...
         * 调大 → 穿透更久；调小 → 更快消散。
         */
        public static int PROJECTILE_MAX_ENTITY_HITS = 14;

        /**
         * 彗星命中判定相对目标碰撞箱的外扩（方块）。
         * 调大 → 更容易在螺旋扫过时连续挂上目标；调小 → 更挑准心。
         */
        public static float PROJECTILE_HIT_INFLATION_BLOCKS = 0.55f;
         ...
         */
        public static double SPIRAL_ORBIT_RADIUS_BLOCKS = 0.65;

        /**
         * 绕轴角速度（度/tick），对应欧拉相位 θ。
         * 调大 → 互旋更快、螺旋更密；调小 → 大螺距、旋转更慢。
         */
        public static float SPIRAL_ANGULAR_SPEED_DEGREES_PER_TICK = 28.0f;

        /**
         * 出手后螺旋半径从 0 涨到满值的 tick 数，避免出生点两星重叠穿模。
         * 调大 → 展开更慢；0 = 立刻满半径。
         */
        public static int SPIRAL_RADIUS_RAMP_TICKS = 3;

        // -------------------------------------------------------------------------
        // 彗星头视觉（单颗略小于魔砾，强调「双星」）
        // -------------------------------------------------------------------------

        public static float COMET_HEAD_BODY_SCALE = 0.34f;
        public static float COMET_HEAD_GLOW_SCALE = 0.68f;
        public static float COMET_HEAD_GLOW_PULSE_AMPLITUDE = 0.09f;
        public static float COMET_HEAD_GLOW_SPIN_DEGREES_PER_TICK = 24.0f;

        public static float COMET_HEAD_CORE_RED = 0.18f;
        public static float COMET_HEAD_CORE_GREEN = 0.90f;
        public static float COMET_HEAD_CORE_BLUE = 1.0f;

        public static float COMET_HEAD_GLOW_RED = 0.14f;
        public static float COMET_HEAD_GLOW_GREEN = 0.92f;
        ...
        public static com.eldenring.spells.entity.GlintstoneTrailStyle TRAIL_STYLE =
                new com.eldenring.spells.entity.GlintstoneTrailStyle(32.0, 0.050f, 0.012f, 0.26f, 0.08f, 64);

        /** 拖尾点缀强度倍率；不影响几何光束长宽。 */
        public static float TRAIL_PARTICLE_INTENSITY = 0.70f;
        public static float IMPACT_PARTICLE_INTENSITY = 1.15f;
        public static float CAST_BURST_PARTICLE_INTENSITY = 1.1f;
     ...
     * 「Pierce」沿用铁魔法玩家熟悉的穿透措辞；数字来自本类字段，不是随等级涨。
     */
    @Override
    public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
        return List.of(
                ...
                Component.literal("Pierce ×" + SpiralShardSpell.PROJECTILE_MAX_ENTITY_HITS)
        );
    }
    ...
    public void onCast(
        ...
        if (!level.isClientSide) {
            ...
            GlintstoneCastHelper.spawnAlongLook(
                    ...
                    SpiralShardSpell.PROJECTILE_SPAWN_FORWARD_OFFSET_BLOCKS,
                    SpiralShardSpell.SPELL_CAST_BURST_FORWARD_OFFSET_BLOCKS,
                    SpiralShardSpell.CAST_BURST_PARTICLE_INTENSITY,
