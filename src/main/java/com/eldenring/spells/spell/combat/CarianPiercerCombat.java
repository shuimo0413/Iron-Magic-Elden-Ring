...
/**
 * 卡利亚贯刺命中：以施法者朝向为轴的扇形。半径 / 半角 / 击退读 {@link CarianPiercerSpell}。
 */
public final class CarianPiercerCombat {
    ...
    public static void resolveSlash(
            CarianPiercerEntity piercerEntity,
            Level level,
            float slashDamage
    ) {
        CarianPiercerSpell spell = (CarianPiercerSpell) ModSpells.CARIAN_PIERCER.get();
        Entity owner = piercerEntity.getOwner();
        if (!(owner instanceof LivingEntity livingOwner) || !livingOwner.isAlive()) {
            return;
        }

        float radiusBlocks = CarianPiercerSpell.SLASH_RADIUS_BLOCKS;
        float halfAngleDegrees = CarianPiercerSpell.SLASH_HALF_ANGLE_DEGREES;
        ...
        var damageSource = spell.getDamageSource(piercerEntity, livingOwner);
        ...
        for (LivingEntity target : level.getEntitiesOfClass(LivingEntity.class, searchBox, candidate ->
            ...
            double knockbackStrength = CarianPiercerSpell.SLASH_KNOCKBACK_STRENGTH;
            target.knockback(
                    knockbackStrength,
