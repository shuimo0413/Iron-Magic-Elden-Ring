...
import com.eldenring.spells.spell.helper.CarianSlicerCasting;
...
/**
 * 卡利亚迅剑（Carian Slicer）：按下出第一刀，长按交替 {@code carian_slicer_1} / {@code carian_slicer_2}。
 * <p>
 * 客户端 PlayerAnimator 斩击动作；每刀 0.5 秒片长，由 {@link com.eldenring.spells.client.CarianSlicerClientHold} 调度。
 * 手里的像素剑由 {@link com.eldenring.spells.client.render.carian.CarianSlicerHandLayer} 跟右手骨骼。
 * 挥砍轨迹 / 刀光 / 闪星由 {@link com.eldenring.spells.spell.fx.CarianSlicerFx} 与
 * {@link com.eldenring.spells.client.render.carian.CarianSlicerTrail} 跟剑走。
 * 服务端斩击锚点 {@link com.eldenring.spells.entity.CarianSlicerEntity} 按刀结算扇形伤害；
...
public class CarianSlicerSpell extends EldenRingAbstractSpell {
     ...
     */
    public static float SLASH_RADIUS_BLOCKS = 3.0f;

    /**
     * 扇形半角（度）。相对视线左右各半角；调大 → 侧面更容易命中。
     */
    public static float SLASH_HALF_ANGLE_DEGREES = 70.0f;

    /**
     * 命中击退强度。调大 → 被砍的怪往后弹得更开。
     */
    public static double SLASH_KNOCKBACK_STRENGTH = 0.25;
    ...
    /**
     * 起手动画由 {@link com.eldenring.spells.client.CarianSlicerClientHold} 在无镜像的专用层播放。
    ...
    public void onServerPreCast(
        ...
        if (!level.isClientSide && playerMagicData != null) {
            ...
            CarianSlicerCasting.ensureSlicerEntity(
                    level,
                    entity,
                    castData,
    ...
    public void onServerCastTick(
        ...
        if (playerMagicData.getAdditionalCastData() instanceof CarianSlicerCastData existingCastData) {
            castData = existingCastData;
        } else {
            castData = new CarianSlicerCastData();
            playerMagicData.setAdditionalCastData(castData);
        }
        CarianSlicerCasting.ensureSlicerEntity(
                level,
    ...
    public void onServerCastComplete(
        ...
        if (playerMagicData.getAdditionalCastData() instanceof CarianSlicerCastData castData) {
            CarianSlicerCasting.requestStop(castData);
        }
